...
## Set to True to enable extra debug logging for flash breakpoints.
LOG_FLASH_BKPT = False
...
class FlashBreakpointProvider(SoftwareBreakpointProvider):
    ...
    def _save_state(self, maxPageSize):
        ...
        if LOG_FLASH_BKPT: self._log.debug("Saved registers: [%s]", " ".join("%08x" % r for r in self._saved_regs))

        start = self._flash.flash_algo['load_address']
        count = self._flash.begin_stack - start
        if LOG_FLASH_BKPT: self._log.debug("Saving algo region [%x+%x]", start, count)
        self._saved_algo = self._core.readBlockMemoryUnaligned8(start, count)

        start = self._flash.begin_data
        if LOG_FLASH_BKPT: self._log.debug("Saving buffer region [%x+%x]", start, maxPageSize)
        self._saved_buffer = self._core.readBlockMemoryUnaligned8(start, maxPageSize)

    def _restore_state(self):
        if LOG_FLASH_BKPT: self._log.debug("Restoring state")
    ...
    def _start_page_updates(self, maxPageSize):
        ...
        # Init flash algo.
        if LOG_FLASH_BKPT: self._log.debug("Initing flash algo")
...
#         return self._breakpoints.get(addr, None)
    ...
    def set_breakpoint(self, addr):
        ...
        if LOG_FLASH_BKPT: self._log.debug("Inserting flash bp @ %x", addr)
    ...
    def remove_breakpoint(self, bp):
        ...
        if LOG_FLASH_BKPT: self._log.debug("Removing flash bp @ %x", bp.addr)
    ...
    def _update_one_page(self, page):
        try:
            ...
            if LOG_FLASH_BKPT: self._log.debug("Reading original page [%x+%x]", start, page.info.size)
            ...
            for op in page.ops:
                ...
                if LOG_FLASH_BKPT: self._log.debug("Changing [%x:%x] from 0x%02x to 0x%02x", start + offset, start + offset + 2,
            ...
            # Erase the page.
            if LOG_FLASH_BKPT: self._log.debug("Erasing page @ %x", start)
            self._flash.erasePage(start)

            # Program the page.
            if LOG_FLASH_BKPT: self._log.debug("Programming page @ %x", start)
            ...
            if ENABLE_VERIFY:
                ...
                if verifyData != pageData:
                    if LOG_FLASH_BKPT: self._log.error("Verify failed")
                    for i in range(len(verifyData)):
                        if verifyData[i] != pageData[i]:
                            if LOG_FLASH_BKPT: self._log.error("Mismatch at byte %d: expected=0x%02x actual=0x%02x",
                                i, pageData[i], verifyData[i])
                else:
                    if LOG_FLASH_BKPT: self._log.debug("Page verified")
    ...
    def flush(self):
        try:
            ...
            if LOG_FLASH_BKPT: self._log.debug("flash bp updates: %s", repr(pages))
