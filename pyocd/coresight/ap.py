...
class APAddressBase(object):
    """! @brief Abstract base class for AP addresses.
    
    Defines the abstract _address_ property, and hash and ordering methods.
    
    In addition to carrying the AP address, the class defines which version of AP is targeted:
    either APv1 or APv2. The _ap_version_ property reports this version number. The AP version is
    coupled with the address because the two are intrinsically connected; the version defines the
    address format.
    """
...
class APv1Address(APAddressBase):
    ...
    An APv1 is identified by an 8-bit APSEL value. This is written into the top byte of
    the DP SELECT register to select the AP to communicate with.
    """
    def __init__(self, apsel):
        self._apsel = apsel
...
class APv2Address(APAddressBase):
    ...
    ADIv6 uses an APB bus to communicate with APv2 instances. The APB bus address width is
    variable from 12-52 bits in 8-bit steps. This address is written the DP SELECT and possibly
    SELECT1 (for greater than 32 bit addresses) registers to choose the AP to communicate with.
    """
