...
class SWDProtocol(object):
    def __init__(self):
        self._trn = 1
        self._trnBits = bitstring.ones(self._trn) # Single-bit turnaround by default
        self._trnDirBits = bitstring.zeros(self._trn) # Direction bits for turnaround.
        self._ackBits = bitstring.bitstring('111') % self._trnBits
        self._ackDirBits = bitstring.bitstring('000') % self._trnDirBits
    
    def generate_request(self, APnDP, RnW, A32):
        # 0     1     2   3  4  5      6    7
        # Start APnDP RnW A2 A3 Parity Stop Park
        pkt = bitstring.bitstring('10000001')
        pkt[1] = APnDP
        pkt[2] = RnW
        pkt[3] = (A32 >> 1) & 1
        pkt[4] = A32 & 1
        parityValue = bitstring.zero_or_one(APnDP) % bitstring.zero_or_one(RnW) % bitstring.bitstring(A32, width=2)
        pkt[5] = parityValue.bit_count() & 1 # parity = 1 if 1s count is odd
        
        return pkt
    
    def generate_wdata(self, data):
        pkt = bitstring.bitstring(data, width=32)
        pkt %= bitstring.zero_or_one(pkt.bit_count() & 1)
        return pkt

    def generate_write(self, APnDP, A32, data):
        # Write request.
        swdio = self.generate_request(APnDP, 0, A32)
        dir = bitstring.ones(8)
        ...
        dir %= bitstring.ones(33)
        
        return swdio.reverse(), dir.reverse()
    
    def extract_write_ack(self, data):
        bits = bitstring.bitstring(data, width=(8 + self._trn + 3 + self._trn + 33))
    ...
    def generate_read(self, APnDP, A32):
        ...
        swdio %= bitstring.ones(36) % self._trnBits
        dir %= bitstring.zeros(36) % self._trnDirBits
        
        return swdio.reverse(), dir.reverse()
    
    def extract_read_result(self, data):
        bits = bitstring.bitstring(data, width=36)
        ack = bits[0:4]
        value = bits[4:35]
        parity = bits[35:36]
